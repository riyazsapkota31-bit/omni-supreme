...
function loadFeedbackData() {
    ...
    if (saved) {
        try { tradeHistory = JSON.parse(saved); } catch(e) {}
    }
    
    const savedWeights = localStorage.getItem('omni_strategy_weights');
    if (savedWeights) {
        try { strategyWeights = JSON.parse(savedWeights); } catch(e) {}
    }
...
function addOpenTrade(signal, marketData, tradeLevels) {
    const trade = {
        ...
        strategy: signal.primaryStrategy || 'Signal Confluence',
...
// Get open trades for auto-tracking
function getOpenTradesForTracking() {
...
function recordFeedback(tradeId, outcome, notes = '') {
    ...
    trade.feedbackNotes = notes;
    trade.closedAt = new Date().toISOString();
    trade.feedbackGiven = true;
    ...
    switch(outcome) {
        ...
        case 'PARTIAL':
            weightChange = -0.01;
            outcomeMessage = '⚠️ PARTIAL - TP touched then SL';
            break;
        case 'REVERSAL':
            weightChange = -0.03;
            outcomeMessage = '📉 REVERSAL - Toward TP then SL';
            break;
        case 'LOSS':
            weightChange = -0.03;
            outcomeMessage = '❌ LOSS';
    ...
    showToastMessage(`${outcomeMessage} recorded for ${trade.symbol}`, outcome === 'WIN' ? 'success' : 'warning');
}
...
function getAdjustedConfidence(signal) {
    let weight = 1.0;
    if (strategyWeights[signal.primaryStrategy]) {
        weight = strategyWeights[signal.primaryStrategy];
    }
    let adjusted = signal.confidence * weight;
    
    const similarTrades = tradeHistory.filter(t => 
        t.strategy === signal.primaryStrategy && 
        t.status === 'CLOSED'
    ).slice(0, 20);
...
function renderOpenTrades() {
    ...
    if (openTrades.length === 0) {
        container.innerHTML = '<p class="text-center text-slate-500 text-xs py-4">No open trades. Run analysis and trade will appear here.</p>';
        return;
    }
    ...
    container.innerHTML = openTrades.map(trade => `
        <div class="bg-black/30 rounded-xl p-3 mb-2 border-l-4 ${trade.bias === 'BUY' ? 'border-emerald-500' : 'border-rose-500'}">
            <div class="flex justify-between items-center mb-2">
                <div>
                    <span class="font-bold ${trade.bias === 'BUY' ? 'text-emerald-400' : 'text-rose-400'}">${trade.bias}</span>
                    <span class="text-xs ml-2">${trade.symbol}</span>
                    <span class="text-[9px] ml-2 text-slate-500">${new Date(trade.timestamp).toLocaleTimeString()}</span>
                </div>
            ...
            <div class="grid grid-cols-2 gap-2">
                <button onclick="window.recordFeedback(${trade.id}, 'WIN')" class="bg-emerald-500/20 text-emerald-400 px-2 py-1.5 rounded-lg text-[9px] hover:bg-emerald-500/30 transition">
                    ✅ WIN (TP hit)
                </button>
                <button onclick="window.recordFeedback(${trade.id}, 'PARTIAL')" class="bg-yellow-500/20 text-yellow-400 px-2 py-1.5 rounded-lg text-[9px] hover:bg-yellow-500/30 transition">
                    ⚡ PARTIAL (TP → SL)
                </button>
                <button onclick="window.recordFeedback(${trade.id}, 'REVERSAL')" class="bg-orange-500/20 text-orange-400 px-2 py-1.5 rounded-lg text-[9px] hover:bg-orange-500/30 transition">
                    🔄 REVERSAL (Toward TP → SL)
                </button>
                <button onclick="window.recordFeedback(${trade.id}, 'LOSS')" class="bg-rose-500/20 text-rose-400 px-2 py-1.5 rounded-lg text-[9px] hover:bg-rose-500/30 transition">
                    ❌ LOSS (SL hit)
                </button>
            </div>
        </div>
...
function renderFeedbackHistory() {
    ...
    const closedTrades = tradeHistory.filter(t => t.status === 'CLOSED').slice(0, 30);
    
    if (closedTrades.length === 0) {
        container.innerHTML = '<p class="text-center text-slate-500 text-xs py-4">No feedback recorded yet. Add feedback to improve signals.</p>';
        return;
    }
    
    const wins = closedTrades.filter(t => t.result === 'WIN').length;
    const partials = closedTrades.filter(t => t.result === 'PARTIAL').length;
    const reversals = closedTrades.filter(t => t.result === 'REVERSAL').length;
    ...
    container.innerHTML = `
        <div class="grid grid-cols-5 gap-1 mb-3 text-center text-[9px]">
            ...
            <div class="bg-indigo-500/10 rounded p-1"><span class="text-indigo-400">TOTAL</span><br>${closedTrades.length}</div>
        </div>
        ${closedTrades.slice(0, 10).map(trade => `
            <div class="bg-black/30 rounded-lg p-2 mb-1 text-[10px]">
                <div class="flex justify-between items-center">
                    <div>
                        <span class="${trade.result === 'WIN' ? 'text-emerald-400' : trade.result === 'PARTIAL' ? 'text-yellow-400' : trade.result === 'REVERSAL' ? 'text-orange-400' : 'text-rose-400'} font-bold">
                            ${trade.result}
                        </span>
                        <span class="text-slate-500 ml-2">${trade.symbol}</span>
                    </div>
...
function updateStrategyPerformance() {
    ...
    const performance = Object.entries(strategyWeights).slice(0, 5);
    
    if (performance.length === 0) {
        container.innerHTML = '<p class="text-center text-slate-500 text-xs py-4">No performance data yet.</p>';
        return;
    }
    
    container.innerHTML = performance.map(([name, weight]) => `
...
function clearAllFeedback() {
    if (confirm('Clear all feedback history? This will reset strategy weights.')) {
        tradeHistory = [];
        STRATEGY_TYPES.forEach(s => { strategyWeights[s] = 1.0; });
        saveFeedbackData();
        renderOpenTrades();
