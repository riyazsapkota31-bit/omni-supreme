/**
 * MARKET DATA FETCHER - FINAL WORKING VERSION
 * Uses CORS proxy for Alpha Vantage, plus fallbacks to Twelve Data, Binance, Yahoo.
 */
...
const MarketData = {
    ...
    async fetch(xmSymbol) {
        ...
        // 1. Alpha Vantage (via CORS proxy – works from any browser)
        if (this.getAlphaKey()) {
            try {
                const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(
                    `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${xmSymbol}&apikey=${this.alphaKey}`
                )}`;
        ...
        // 2. Twelve Data (demo key – no CORS issues)
        try {
        ...
        // 3. Binance (crypto only – real‑time)
        ...
        // 4. Yahoo Finance (last resort – delayed)
        try {
